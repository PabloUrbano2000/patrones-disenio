...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(`%cAtención de $/${amount} por el supervisor`, COLORS.blue)
      return
    }
    this.next(amount)
  }
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(`%cAtención de $/${amount} por el manager`, COLORS.brown)
      return
    }
    this.next(amount)
  }
...
class Director extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    console.log(`%cAtención de $/${amount} por el director`, COLORS.green)
  }
}
